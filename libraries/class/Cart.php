...
class Cart
{
    ...
    public function getProductInfo($pid = 0)
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQueryOne("select * from table_product where id = ? limit 0,1", array($pid));
        }
        return $row;
    }
    ...
    public function getProductColor($color = 0)
    {
        $str = '';
        if ($color) {
            $row = $this->d->rawQueryOne("select * from table_color where id = ? limit 0,1", array($color));
            $str = $row['name'];
        }
        return $str;
    }
    ...
    public function getProductSize($size = 0)
    {
        $str = '';
        if ($size) {
            $row = $this->d->rawQueryOne("select name from table_size where id = ? limit 0,1", array($size));
            $str = $row['name'];
        }
        return $str;
    }
