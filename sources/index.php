...
$brand = $d->rawQuery("select * from table_brand where find_in_set('hienthi',status) order by id desc", array());
...
$productlist = $d->rawQuery("select * from table_list where find_in_set('noibat',status) and find_in_set('hienthi',status) order by id desc", array());
