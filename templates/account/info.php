                ...
                <?php if ($config['user']['info'] == true) { ?>
                    <div class="container_load_info load2">
                        ...
                        <?php if (array_key_exists($loginMember, $_SESSION) && $_SESSION[$loginMember]['active'] == true) { ?>
                            ...
                            $donhang = $d->rawQuery("select code, order_status, total_price, id, date_created from table_order where id_user = ?", array($iduser));
                            if (!empty($donhang)) { ?>
                                <?php foreach ($donhang as $v1) {
                                    $chitiet = $d->rawQuery("select photo,name,regular_price,sale_price,quantity,id_product from table_order_detail where id_order = ?", array($v1['id']));
                                    $tinhtrang = $d->rawQueryOne("select namevi from #_order_status where id = ? limit 0,1", array($v1['order_status']));
                                ?>
                                    <div class="wrap-cart box-ql-donhang">
                                        <div class="top-cart">
                                            <div class="title-cart">
                                                <p>Đơn hàng: <span><?= $v1['code'] ?></span></p>
                                                <p>Tình trạng: <span><?= $tinhtrang['namevi'] ?></span></p>
                                            </div>
                                                ...
                                                <table class="table">
                                                    ...
                                                    ?>
                                                        <tbody>
                                                            ...
                                                            <th>
                                                                <h3 class="name-procart"><a class="text-decoration-none" target="_blank" title="<?= $v2['name'] ?>"><?= $v2['name'] ?></a></h3>
                                                            </th>
